...
class GoogLeNet(nn.Module):
    def __init__(self, num_classes=1000, aux_logits=True):
        ...
        # 命名为 Inception(in_channels, 1x1, 3x3red, 3x3, 5x5red, 5x5, pool_proj)
    ...
    def forward(self, x):
        ...
        # Inception 4 + Aux Classifier 1
        x = self.inception4a(x)
        aux1 = self.aux1(x) if self.training and self.aux_logits else None # 训练时使用辅助分类器 1
        x = self.inception4b(x)
        x = self.inception4c(x)
        x = self.inception4d(x)
        aux2 = self.aux2(x) if self.training and self.aux_logits else None # 训练时使用辅助分类器 2
...
class InceptionAux(nn.Module):
    def __init__(self, in_channels, num_classes):
        ...
        self.avgpool = nn.AvgPool2d(kernel_size=5, stride=3) # 原始论文的池化层
        self.conv = BasicConv2d(in_channels, 128, kernel_size=1)
        
        self.fc1 = nn.Linear(128 * 4 * 4, 1024) # 假设输入图像为 224x224，池化后大小为 4x4
